...
namespace Morgobot.Test.Brain
{
    ...
    public class MovementAnalyzerTest
    {
        ...
        {
            Assert.AreEqual("Я в зале, тут тепло и кроватка.", _movementThoughts.Analyse(new Phrase("где ты")));
            Assert.AreEqual("Поднимаю подушку и вижу под ней забытую бутылку пива, которую хотел выпить на ночь. Ура! Я нашел 1 из 7 пив!", _movementThoughts.Analyse(new Phrase("посмотри под подушкой")));
            Assert.AreEqual("Я в коридоре, ничего интересного. На полу лежит ковер.", _movementThoughts.Analyse(new Phrase("назад")));
            Assert.AreEqual("Поднимаю ковер и вижу что что-то блеснуло. Это пиво! Ура! Я нашел 2 из 7 пив!", _movementThoughts.Analyse(new Phrase("посмотри под ковром")));
            Assert.AreEqual("Я в кладовке, темно и уютно.", _movementThoughts.Analyse(new Phrase("налево")));
            Assert.AreEqual("В темноте я вижу кучу пустых бутылок. Порывшись в ней я нахжу одну полную. Ура! Я нашел 3 из 7 пив!", _movementThoughts.Analyse(new Phrase("включи свет")));
            Assert.AreEqual("Я в коридоре, ничего интересного. На полу лежит ковер.", _movementThoughts.Analyse(new Phrase("направо")));
            Assert.AreEqual("Не пойду, лифт не работает!", _movementThoughts.Analyse(new Phrase("назад")));
            Assert.AreEqual("Я в ванной, горячей воды нет.", _movementThoughts.Analyse(new Phrase("направо")));
